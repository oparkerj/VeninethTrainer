...
public abstract class MemoryWatcher
{
    ...
    {
        if (!UpdateInterval.HasValue) return true;
...
public class StringWatcher : MemoryWatcher
{
    ...
    {
        get => (string) (base.Current ?? string.Empty);
        set => base.Current = value;
    }
    
    public new string Old
    {
        get => (string) (base.Old ?? string.Empty);
        set => base.Old = value;
    }
    ...
    public override bool Update(Process process)
    {
        ...
        var success = AddrType == AddressType.DeepPointer
            ? DeepPtr.DerefString(process, _stringType, _numBytes, out var str)
...
public class MemoryWatcher<T> : MemoryWatcher
    where T : struct
{
